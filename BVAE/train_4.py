...
def train(args, model, sess):
    ...
    best_ndcg5_p, best_ndcg5_cl, best_ndcg5_f, best_ndcg5_cr = 0., 0., 0., 0.
    best_iter_p, best_iter_cl, best_iter_f, best_iter_cr = 0, 0, 0, 0
    ...
    favouriteData_matrix, favouriteDict = loadTrainData(args=args, i=2)
    users_set = users_set.union(set(favouriteDict.keys()))
    cartData_matrix, cartDict = loadTrainData(args=args, i=3)
    users_set = users_set.union(set(cartDict.keys()))
    ...
    favouriteTestDict = loadTestData(args=args, i=2)
    userList.append(sorted(set(favouriteTestDict.keys())))
    cartTestDict = loadTestData(args=args, i=3)
    userList.append(sorted(set(cartTestDict.keys())))
    ...
    for epoch in range(args.epoch):
        ...
        for bnum, st_idx in enumerate(range(0, len(userList_train), args.batch_size)):
            ...
            if sparse.isspmatrix(B2):
                B2 = B2.toarray()
            B3 = favouriteData_matrix[batchIndex]
            if sparse.isspmatrix(B3):
                B3 = B3.toarray()
            B4 = cartData_matrix[batchIndex]
            if sparse.isspmatrix(B4):
                B4 = B4.toarray()
            D = B1 + B2 + B3 + B4
            ...
            feed_dict = {model.input_ph_union: D, model.input_ph_behavior1: B1, model.input_ph_behavior2: B2,
                         model.input_ph_behavior3: B3, model.input_ph_behavior4: B4,
        ...
        for task_i in range(args.task_num):
            ...
            for bnum, st_idx in enumerate(range(0, len(userList_test), args.batch_size)):
                ...
                if task_i == 0:
                    ...
                    allRatings_batch[B3.nonzero()] = -np.inf
                    for i in batchIndex:
                        testDict_batch.append(favouriteTestDict[i])
                elif task_i == 3:
                    allRatings_batch[B4.nonzero()] = -np.inf
                    for i in batchIndex:
                        testDict_batch.append(cartTestDict[i])
        ...
        if best_ndcg5_f < ndcg5[2]:
            best_iter_f = epoch
            best_ndcg5_f = ndcg5[2]
        print("Best NDCG@5_favourite = %.4f when %d-th epoch" % (best_ndcg5_f, best_iter_f))
        logger.info("Best NDCG@5_favourite = %.4f when %d-th epoch" % (best_ndcg5_f, best_iter_f))
        if best_ndcg5_cr < ndcg5[3]:
            best_iter_cr = epoch
            best_ndcg5_cr = ndcg5[3]
        print("Best NDCG@5_cart = %.4f when %d-th epoch" % (best_ndcg5_cr, best_iter_cr))
        logger.info("Best NDCG@5_cart = %.4f when %d-th epoch" % (best_ndcg5_cr, best_iter_cr))
